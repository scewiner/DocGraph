...
# @File: doc_graph.py.py
# @Software: PyCharm
# @Time : 2021/3/27 21:53
# @Author : Scewiner, Xu, Mingzhou
# @File: multi-LB.py
# @Software: PyCharm
...
# from .GraphLayer import GraphEncoders as GraphEncoder
from .GraphLayers import GraphEncoderT as GraphEncoder
from torch import Tensor
from torch.nn.utils.rnn import pad_sequence
DEFAULT_MAX_SOURCE_POSITIONS = 1024
DEFAULT_MAX_TARGET_POSITIONS = 1024
DEFAULT_MAX_DOC_POSITIONS = 3000
...
@register_model("DGFoward")
class DGFowardModel(FairseqEncoderDecoderModel):
    """
    ...
    def add_args(parser):
        ...
        # zhang et al.2018
        parser.add_argument('--ctx-layer', default=2, type=int, help='num of ctx layers')
        parser.add_argument('--shared-layers', default=False, action='store_true', help='share')
        parser.add_argument('--share-ctx-layer', default=False, action='store_true', help='share')
        parser.add_argument('--typeatt', default=False, action='store_true', help='typeattn')
        parser.add_argument(
    ...
    def forward(
        ...
        encoder_out = self.encoder(
            ...
            return_all_hiddens=return_all_hiddens, graph=graph, ctx=ctx,doc_id=doc_id,
            sent_id=sent_id,
        )
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.cross_layer = self.build_encoder_attention(embed_dim,args)
        self.cross_attn_layer_norm = LayerNorm(embed_dim)
        self.gate = GatingResidual(embed_dim)
        self.sentence_embedding = Embedding(DEFAULT_MAX_DOC_POSITIONS,embed_dim,padding_idx=None)
    ...
    def forward_scriptable(
        ...
        ctxs = []
        ll = []
        # if batch contains more graph from different documents.
        for c,g in zip(ctx,graph):
            tmp_mask = c.eq(self.padding_idx)
            ctx_ = self.forward_embedding(c, token_embeddings)
            # subword 2 word
            ctx_ = ctx_.masked_fill(tmp_mask.unsqueeze(-1),value=0.0).mean(1)
            ctx_ = self.coder(ctx_, g)  # T x C
            ll.append(ctx_.size(0))
            ctxs.append(ctx_.squeeze(1))
            
        ctxs = pad_sequence(ctxs,batch_first=True,padding_value=0.0)
        for i in range(len(ll)):
            ll[i] = torch.ones((ll[i]),dtype=torch.int,device=x.device)
        ll = pad_sequence(ll,batch_first=True,padding_value=self.padding_idx)
        l = ll.size(1)
        sent_order = torch.arange(l,dtype=torch.long,device=x.device)
        ctxs += self.sentence_embedding(sent_order)
        # ctxs should be doc_num x seq x dim, external doc_num to bzs
        ctxs = torch.index_select(ctxs,dim=0,index=doc_id)
        ll = torch.index_select(ll,dim=0,index=doc_id).bool()
        
        sent_order_ = self.sentence_embedding(sent_id)
        sent_order_ = sent_order_.unsqueeze(1)

        assert ctxs.size(0) == x.size(0)
        # B x T x C -> T x B x C
        x += sent_order_
        x = x.transpose(0, 1)
        ctxs = ctxs.transpose(0,1)
        residual = x
        if self.normalize_before:
            x = self.cross_attn_layer_norm(x)
        
        x, _ = self.cross_layer(
            query=x,
            key=ctxs,
            value=ctxs,
            key_padding_mask=~ll,
    ...
    def reorder_encoder_out(self, encoder_out: Dict[str, List[Tensor]], new_order):
        ...
        if len(encoder_out["sent_order"]) == 0:
            new_sent_order_ = []
        else:
            new_sent_order_ = [
                encoder_out["sent_order"][0].index_select(0, new_order)
            ]

        if len(encoder_out["doc_id"]) == 0:
            new_doc_id = []
        else:
            new_doc_id = [
                encoder_out["doc_id"][0].index_select(0, new_order)
            ]
        ...
        return {
            ...
            "sent_order": new_sent_order_,  # List[T x B x C]
            "doc_id":new_doc_id,
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        super().__init__(dictionary)
        self.register_buffer("version", torch.Tensor([3]))
        self.src_as_tgt = getattr(args,'src_as_tgt',False)
    ...
    def extract_features_scriptable(
        ...
        # B x T x C -> T x B x C
        x += encoder_out['sent_order'][0]
        x = x.transpose(0, 1)
        residual = x
        if self.normalize_before:
            x = self.cross_attn_layer_norm(x)

        # cross-attn for doc-level context

        x, _ = self.cross_layer(
                query=x,
                key=encoder_out['ctx'][0],
                value=encoder_out['ctx'][0],
                key_padding_mask=encoder_out['ctx_mask'][0],
                need_weights=False,
                attn_mask=None
            )
...
@register_model_architecture("DGFoward", "DGFoward_tiny")
...
@register_model_architecture("DGFoward", "DGFoward")
...
@register_model_architecture("DGFoward", "DGFoward_iwslt_de_en")
def DGFoward_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
...
@register_model_architecture("DGFoward", "DGFoward_wmt_en_de")
def DGFoward_wmt_en_de(args):
    base_architecture(args)


# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture("DGFoward", "DGFoward_vaswani_wmt_en_de_big")
def DGFoward_vaswani_wmt_en_de_big(args):
