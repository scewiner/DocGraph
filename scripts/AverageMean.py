...
from .GraphLayer import GraphEncoder
from torch import Tensor

DEFAULT_MAX_SOURCE_POSITIONS = 1024
DEFAULT_MAX_TARGET_POSITIONS = 1024
DEFAULT_MAX_DOC_POSITIONS = 512
...
@register_model("AverageMean")
class AverageMeanModel(FairseqEncoderDecoderModel):
    """
    ...
    def forward(
        ...
        encoder_out = self.encoder(
            src_tokens, src_lengths=src_lengths, return_all_hiddens=return_all_hiddens, graph=graph, ctx=context,
        )
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward_scriptable(
        ...
        # if not ctx_mask.any():
        #    ctx_mask = None
        ctx = self.forward_embedding(ctx, token_embeddings)

        ctx = self.coder(ctx, graph)  # T x C
        l = ctx.size(0)
        sent_order = torch.arange(l,dtype=torch.long(),device=ctx.device)
        ctx += self.sentence_embedding(sent_order)
        # ctx = ctx.expand(x.size(0), 1, 1)
        sent_order_ = self.sentence_embedding(doc_id)
        # B x T x C -> T x B x C
        x +=sent_order_
        doc_represent = torch.index_select(ctx, dim=0, index=doc_id)
        x = torch.cat([doc_represent,x],dim=0)
        x = x.transpose(0, 1)
...
@register_model_architecture("AverageMean", "AverageMean_tiny")
...
@register_model_architecture("AverageMean", "AverageMean")
...
@register_model_architecture("AverageMean", "AverageMean_iwslt_de_en")
def AverageMean_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
...
@register_model_architecture("AverageMean", "AverageMean_wmt_en_de")
def AverageMean_wmt_en_de(args):
    base_architecture(args)


# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture("AverageMean", "AverageMean_vaswani_wmt_en_de_big")
def AverageMean_vaswani_wmt_en_de_big(args):
